...
class clientType:
    ...
    def decode_key(self, valu):
        try:
            ...
            if ':' in dec_ip:
                dec_ip, dec_port = dec_ip.rsplit(':', 1)
                self.PORT = int(dec_port)
            elif len(dec_ip) == 8:
                ...
                self.passkey = input(" Re-enter your accesskey : ")
                dec_ip = self.decode_key(self.passkey)
        except (ConnectionRefusedError, UnicodeDecodeError, UnboundLocalError, ValueError, base64.binascii.Error):
            print("Please enter the correct passkey \n ")
            self.passkey = input(" Re-enter your accesskey : ")
            dec_ip = self.decode_key(self.passkey)
        finally:
