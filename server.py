...
class serverType:
    ...
    def getpasskey(self, str1):
        self.encodefunc("{}:{}".format(str1, self.PORT))
        ...
        while self.PORT <= self.MAX_PORT:
            try:
                ...
                if getattr(error, 'winerror', None) == 10048 or getattr(error, 'errno', None) == 98:
                    print("Port {} is already in use, trying {}...".format(self.PORT, self.PORT + 1))
                    self.PORT += 1
                else:
                    raise
        raise OSError("No available port found between 5050 and {}".format(self.MAX_PORT))
    ...
    def mainFunc(self):
        ...
        thread2.start()
        print("Server created Successfully on {}:{}!\n".format(self.IP, self.PORT))
        self.accept_conn()
